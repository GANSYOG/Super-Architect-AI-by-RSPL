...
const appData = {
    ...
    architecturalStyles: [
        ...
        "Mediterranean", "Colonial", "Art Deco", "Brutalist", "Neo-Classical", "Sustainable/Green"
    ],
    materials: [
        "Exposed Concrete", "Natural Stone", "Hardwood", "Steel Frame", 
        "Glass Curtain Wall", "Brick", "Bamboo", "Rammed Earth", "Cor-Ten Steel", "Recycled Materials"
    ],
    specialFeatures: [
        "Rooftop Garden", "Solar Panels", "Swimming Pool", "Home Theater", 
        "Wine Cellar", "Smart Home Integration", "Elevator", "Fireplace", "Skylight", "Balcony/Terrace"
    ],
    spaceTypes: {
    ...
    promptTemplates: [
        {
            ...
            template: "Design a luxury 3-bedroom apartment with contemporary styling. Include premium materials like marble countertops, hardwood flooring, and smart home integration. Focus on open-plan living with floor-to-ceiling windows and modern lighting throughout."
        },
        {
            id: 2,
            ...
            template: "Create a modern corporate headquarters featuring open workspaces, private offices, conference rooms, and collaborative areas. Include sustainable materials, natural lighting, and technology integration throughout the space."
        },
        {
            id: 3,
            ...
            template: "Design a sustainable 4-bedroom family home incorporating solar panels, rainwater harvesting, natural ventilation, and locally sourced materials. Include outdoor spaces and gardens integrated with the living areas."
        },
        {
            id: 4,
            ...
            template: "Create a boutique hotel with 20 rooms featuring unique design elements, local cultural influences, and premium amenities. Include a restaurant, lobby lounge, and spa facilities with sustainable design principles."
        },
        {
            id: 5,
            ...
            template: "Transform an industrial warehouse into a modern loft apartment. Preserve original features like exposed brick and steel beams while adding contemporary elements like mezzanine levels and large windows."
        }
    ],
    sampleProject: {
        ...
        description: "A stunning contemporary villa featuring clean lines, natural materials, and panoramic lake views. The design emphasizes indoor-outdoor living with expansive glass walls and tropical landscaping.",
        images: {
                ...
                <div class="section-content">
                    <div class="form-group">
                        ...
                        <div id="styles-selection" class="selection-grid">
                            ${appData.architecturalStyles.map(style => `
                    ...
                    <div class="form-group">
                        ...
                        <div id="features-selection" class="selection-grid">
                            ${appData.specialFeatures.map(feature => `
...
function setupProjectForm() {
    ...
    document.querySelectorAll('[name="project-type"]').forEach(radio => {
        radio.addEventListener('change', (e) => {
            ...
            subtypeOptions.innerHTML = appData.spaceTypes[selectedType].map(subtype => `
            ...
            document.querySelectorAll('[name="space-subtype"]').forEach(subtypeRadio => {
...
function collectFormData() {
    const formData = {
        ...
        styles: Array.from(document.querySelectorAll('#styles-selection input:checked')).map(cb => cb.value),
        materials: Array.from(document.querySelectorAll('#materials-selection input:checked')).map(cb => cb.value),
        features: Array.from(document.querySelectorAll('#features-selection input:checked')).map(cb => cb.value),
        dimensions: {
        ...
        case 'images':
            content.innerHTML = `
                <div class="image-gallery">
                    ...
                    ${project.images.exterior.map((img, index) => `
                    ...
                    ${project.images.interior.map((img, index) => `
                        <div class="image-item">
        ...
        case 'sustainability':
            content.innerHTML = `
                ...
                <div class="eco-section">
                    ...
                    <ul class="eco-list">
                        ${project.sustainability.positives.map(positive => `
                ...
                <div class="eco-section">
                    ...
                    <ul class="eco-list">
                        ${project.sustainability.improvements.map(improvement => `
...
function applyImageEdit(originalSrc, prompt, type, index) {
    ...
    document.getElementById('apply-edit').innerHTML = '<i class="fas fa-spinner fa-spin"></i> Processing...';
    ...
    setTimeout(() => {
        ...
        document.getElementById('apply-edit').innerHTML = '<i class="fas fa-magic"></i> Apply Changes';
        document.getElementById('apply-edit').disabled = false;
        
        document.getElementById('accept-edit').onclick = () => {
            // Update the project image
            if (type === 'exterior') {
                appState.generatedProject.images.exterior[index] = '/api/placeholder/800/600';
            } else {
                appState.generatedProject.images.interior[index] = '/api/placeholder/800/600';
            }
            ...
            <div id="comparison-container" style="display: none;">
                ...
                <div class="shopping-list">
                    ...
                    <div id="furniture-list">
                        ${appData.furnitureItems.map(item => `
...
function setupInteriorRevamp() {
    ...
    document.getElementById('generate-revamp').addEventListener('click', generateRevamp);
...
function tryOnItem(itemId) {
    ...
    showToast(`Virtual try-on for "${item.name}" activated! Drag to place in your room.`);
...
function renderTemplateCards() {
    return appData.promptTemplates.map(template => `
...
function generateAvatar() {
    ...
    setTimeout(() => {
        document.getElementById('generated-avatar').src = '/api/placeholder/150/150';
        ...
        document.getElementById('save-avatar').onclick = () => {
            ...
            const savedAvatars = JSON.parse(localStorage.getItem('customAvatars') || '{}');
...
setInterval(() => {
    if (appState.generatedProject) {
        localStorage.setItem('currentProject', JSON.stringify(appState.generatedProject));
    }
